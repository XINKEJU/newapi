	...
	return func(c *gin.Context) {
		...
		if len(allowIps) > 0 {
			...
			if common.IsIpInCIDRList(ip, allowIps) == false {
				abortWithOpenAiMessage(c, http.StatusForbidden, "Your IP is not in the token's allowed IP list", types.ErrorCodeAccessDenied)
				return
			}
...
func SetupContextForToken(c *gin.Context, token *model.Token, parts ...string) error {
	...
	if len(parts) > 1 {
		if model.IsAdmin(token.UserId) {
			...
			abortWithOpenAiMessage(c, http.StatusForbidden, "Channel specification is not supported for regular users")
			return fmt.Errorf("channel specification is not supported for regular users")
		}
	}
	return nil
}
